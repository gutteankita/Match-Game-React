...
import MatchGameNavbar from './components/MatchGameNavbar'
import MatchGameTabItem from './components/MatchGameTabItem'
...
class App extends Component {
  ...
  getRandomImg = category => {
    const randomIndex = Math.floor(Math.random() * imagesList.length)
    return imagesList[randomIndex]?.imageUrl || null
  }

  onMatch = clickedImageId => {
    const {activeTabId, match, score} = this.state

    const clickedImage = imagesList.find(img => img.id === clickedImageId)
    const newRandomImage = this.getRandomImg(activeTabId)

    if (clickedImage.imageUrl === match) {
      this.setState(prevState => ({
        score: prevState.score + 1,
      }))
    } else if (clickedImage.imageUrl !== match) {
      this.setState({
        gameOver: true,
      })
      clearInterval(this.timerId)
  ...
  render() {
    const filteredProjects = this.getfilteredImgList()
    const {activeTabId, match, score, time, gameOver} = this.state

    return (
      <>
        <MatchGameNavbar score={score} time={time} />

        <div className="app-container">
          {gameOver ? (
            <ul className="game-over-container">
              <img
            ...
            <>
              <div>
                <img src={match} className="show-img" alt="imageUrl" />
              </div>
              ...
              <ul className="items-container">
                {filteredProjects.map(imgDetails => (
                  <MatchGameItem
                    key={imgDetails.id}
